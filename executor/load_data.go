...
func (e *LoadDataExecCompressed) getRecordStatus(ctx context.Context,
	...
	if req.NumRows() == 0 {
		return fmt.Errorf("load data task %v not found", e.recordID), false
	}
	row := iter.Begin()
	//get LiveTime
	if !row.IsNull(0) {
		...
		if time.Since(lastLiveTime) > 20*time.Minute {
			return fmt.Errorf("load data task %v keep alive timed out", e.recordID), true
		}
	}
	//get response
	if !row.IsNull(1) {
...
func (e *LoadDataExecCompressed) waitTaskEnd(ctx context.Context) (error, bool) {
	...
	err, taskStatus := e.getRecordStatus(ctx, sqlExecutor)
	if err != nil {
		/*
...
func (e *LoadDataExecCompressed) addTask(ctx context.Context) (error, uint64) {
	sysSession, err := e.getSysSession()
	if err != nil {
		return err, 0
	}
